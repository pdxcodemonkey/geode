...
public class SetClusterConfigurationCommand implements GfshCommand {
  ...
  File getXmlFile() throws IOException {
    ...
    String xmlFileName = CliUtil.bytesToNames(shellBytesData)[0];
    byte[] xmlFileBytes = CliUtil.bytesToData(shellBytesData)[0];
    File file = new File(xmlFileName);
    FileUtils.writeByteArrayToFile(file, xmlFileBytes);
    return file;
  }
  ...
  Set<DistributedMember> findMembers(@CliOption(key = CliStrings.GROUP,
    ...
    if (ClusterConfigurationService.CLUSTER_CONFIG.equals(group)) {
      serversInGroup = CliUtil.getAllNormalMembers(getCache());
    } else {
      serversInGroup = CliUtil.findMembers(new String[] {group}, null);
    }
  ...
  public static class Interceptor extends AbstractCliAroundInterceptor {
    public Result preExecution(GfshParseResult parseResult) {
      String xmlFile = parseResult.getParamValue(XML_FILE);
      ...
      String group = parseResult.getParamValue(CliStrings.GROUP);
      ...
      try {
        fileResult = new FileResult(new String[] {xmlFile});
