...
public class SetClusterConfigurationCommandDUnitTest {

  @Rule
  public LocatorServerStartupRule cluster = new LocatorServerStartupRule();
  ...
  public void noServerToBeginWith() throws IOException {
    commandResult = gfsh.executeAndVerifyCommand(commandWithFile);
    ...
    server1.invoke(() -> {
      Region region =
          LocatorServerStartupRule.serverStarter.getCache().getRegion("regionForCluster");
      assertThat(region).isNotNull();
      assertThat(region.getAttributes().getDataPolicy()).isEqualTo(DataPolicy.REPLICATE);
    });

    // after that, you can not set it anymore without extra options.
    commandResult = gfsh.executeAndVerifyCommandError(commandWithFile);
    assertThat(commandResult.getContent().toString())
        .contains("Can not set the cluster configuration if there are running servers");

    // you can not bounce the servers because they are not vanilla servers
    commandResult =
        gfsh.executeAndVerifyCommandError(commandWithFile + " --configure-running-servers");
    assertThat(commandResult.getContent().toString())
        .contains("Can not configure servers that are already configured");

    // you can only ignore with existing servers
    commandResult = gfsh.executeAndVerifyCommand(commandWithFile + " --ignore-running-servers");
    assertThat(commandResult.getContent().toString())
        .contains("Existing servers are not affected with this configuration change");

    FileUtils.write(xmlFile, GROUP_XML, Charset.defaultCharset());
    // you can set cluster configuration for another group
    commandResult = gfsh.executeAndVerifyCommand(commandWithFile + " --group=groupA");
    assertThat(commandResult.getContent().toString())
        .contains("Successfully set the 'groupA' configuration to the content of");
    ...
    server2.invoke(() -> {
      Region region1 =
          LocatorServerStartupRule.serverStarter.getCache().getRegion("regionForCluster");
      Region region2 =
          LocatorServerStartupRule.serverStarter.getCache().getRegion("regionForGroupA");
  ...
  public void canNotConfigureIfServersAreNotEmpty() throws IOException {
    ...
    server.invoke(() -> {
      LocatorServerStartupRule.serverStarter.getCache()
          .createRegionFactory(RegionShortcut.REPLICATE).create("regionA");
    });

    commandResult =
        gfsh.executeAndVerifyCommandError(commandWithFile + " --configure-running-servers");
    assertThat(commandResult.getContent().toString())
        .contains("Can not configure servers with existing regions: regionA");
  }
  ...
  public void configureVanillaServers() throws IOException {
    ...
    commandResult = gfsh
        .executeAndVerifyCommand(commandWithFile + " --group=groupA --configure-running-servers");
    assertThat(commandResult.getContent().toString())
        .contains("Successfully set the 'groupA' configuration to the content of");
    assertThat(commandResult.getContent().toString())
        .contains("Configure the servers in 'groupA' group");
    assertThat(commandResult.getContent().toString()).contains("Successfully configured server-1");
    ...
    commandResult = gfsh.executeAndVerifyCommandError(
        commandWithFile + " --group=groupB --configure-running-servers");
    assertThat(commandResult.getContent().toString())
        .contains("Can not configure servers that are already configured.");
  }
}
